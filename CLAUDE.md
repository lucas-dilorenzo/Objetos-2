...
- **Smell:** Magic Number — literal numérico sin nombre descriptivo (ej: `0.79`, `5`)
- **Mecánica:** declarar constante `private static final` con nombre descriptivo → reemplazar todas las ocurrencias del literal

#### Inline Temp

- **Smell:** variable temporal asignada una vez y retornada/usada de inmediato — innecesaria
- **Mecánica:** reemplazar cada referencia a la temp por la expresión que se le asignaba → eliminar declaración
- **Distinto de Replace Temp with Query:** Inline Temp elimina la variable *inline* sin crear un método nuevo; Replace Temp with Query la reemplaza por una llamada a un método extraído (útil cuando la expresión se reutiliza o es compleja)

#### Form Template Method

- **Smell:** Duplicate Code en métodos con la misma estructura general pero pasos variables entre subclases
- **Cuándo:** Pull Up Method no es aplicable directamente porque los métodos no son idénticos
- **Mecánica:**
  1. Extract Method para separar las partes variables de las comunes en cada subclase
  2. Pull Up Method para subir las partes idénticas a la superclase
  3. Subir el método principal (template method) a la superclase — declararlo `final`
  4. Declarar `abstract` los pasos obligatorios que varían entre subclases
  5. Para pasos *opcionales* (presentes solo en algunas subclases): usar **hook method** — cuerpo vacío en la superclase, sobreescrito solo donde aplica
- **Hook method:** método concreto con cuerpo vacío (o comportamiento por defecto) en la superclase; las subclases que necesitan ese paso lo sobreescriben con `@Override`
...
| Variable temporal      | Inline Temp (si es trivial) o Replace Temp with Query (si es compleja o reutilizable) |
...
| 7             | Etiquetas                       | Magic Number → Replace Magic Number with Symbolic Constant; Encapsulate Field; Extract Method; Pull Up Method; **Form Template Method** con hook method (`imprimirPrecioSinImpuesto`) |
| 8             | Documentos y estadísticas       | Public Field → Encapsulate Field (con copia defensiva); Temporary Variable → Inline Temp; Duplicate Code → Extract Method (reusar `characterCount()`); bug en promedio (división entera — NO es refactoring corregirlo) |
| 9             | Pedidos                         | Replace Loop with Pipeline; Replace Conditional with Polymorphism (usar **interface** `Pago`, no clase abstracta); Extract+Move Method → `esClienteAntiguo()` en Cliente; Replace Temp with Query + Extract Method → `costoFinal()` |
...
### PATRÓN TEMPLATE METHOD _(en progreso — ver PDF páginas 21-37)_
...
**Relación con Form Template Method:** Form Template Method es el *refactoring* que lleva al patrón Template Method. El resultado final es el mismo patrón: superclase con el algoritmo esqueleto (`final`), subclases con los pasos variables. La diferencia es que Form Template Method *parte de código duplicado existente* y lo reorganiza, mientras que el patrón describe el diseño objetivo.
...
_Última actualización: 2026-04-20 — Agregados refactorings Replace Magic Number, Inline Temp, Form Template Method + hook method; tabla Smells→Refactorings ampliada; ejercicios 7, 8 y 9 corregidos con smells completos; nota sobre Form Template Method vs patrón Template Method_
